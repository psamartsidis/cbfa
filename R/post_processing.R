...
fitted_values_conjugate = function( id, fit, Timings, y, n, k, X ) {
  ...
  if ( D1>0 ) {
    for ( d in 1:D1 ) {
      ...
      # Fitted values 
      for ( b in 1:nDraws ) {
        lin_pred         = fit$f_normal[,c(idx),b] %*% fit$L[id,,b] + X[,id,] %*% fit$beta_normal[,d,b] 
        fit_normal[b,,d] = lin_pred + rnorm( nTimes, 0, sqrt( fit$sigma[id,1,b] ) )
      }
  ...
  if ( D2>0 ) {
    for ( d in 1:D2 ) {
      ...
      # Fitted values 
      for ( b in 1:nDraws ) {
        lin_pred               = exp( fit$f_binom[,c(idx),b] %*% fit$L[id,,b] + X[,id,] %*% fit$beta_binom[,d,b] )
        lin_pred               = lin_pred / (1+lin_pred)
        fit_binom_prob[b,,d]   = lin_pred
        fit_binom_counts[b,,d] = rbinom( nTimes, n[,id,d], lin_pred )
      }
